...
### 폴더 구조 대규모 재편 (Option 1B) 🎉
- **루트 디렉토리 정리**: 39개 → 13개 항목 (67% 감소)
- **assets/ 폴더 신설**: CSS/JS/이미지 통합 관리
  - `assets/css/` - 스타일시트
  - `assets/js/data/` - 데이터 파일 (7개)
  - `assets/js/scripts/` - 스크립트 파일 (4개)
  - `assets/images/` - 이미지 통합 (members, research, software, gallery, icons)
- **docs/ 폴더 신설**: 모든 문서 파일 분리
- **경로 업데이트**: 모든 HTML 파일의 경로 자동 수정
- **효과**: 깔끔하고 전문적인 프로젝트 구조, 유지보수 용이

### 문서 정리
- **MD 파일 통합**: 16개 → 6개로 간소화
- **README 업데이트**: 새 폴더 구조 반영
- **신규 문서**: RESTRUCTURE_GUIDE.md 추가
